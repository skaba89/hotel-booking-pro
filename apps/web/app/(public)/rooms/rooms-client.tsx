...
function sortRooms(list: any[], sortBy: string) {
  const copy = [...list];
  if (sortBy === 'price_asc') return copy.sort((a, b) => Number(a.pricePerNight) - Number(b.pricePerNight));
  if (sortBy === 'price_desc') return copy.sort((a, b) => Number(b.pricePerNight) - Number(a.pricePerNight));
  if (sortBy === 'capacity') return copy.sort((a, b) => b.capacity - a.capacity);
  if (sortBy === 'size') return copy.sort((a, b) => Number(b.sizeM2) - Number(a.sizeM2));
  return copy;
}

export function RoomsClient({ initialRooms, initialCheckIn = '', initialCheckOut = '' }: Props) {
  const [sortBy, setSortBy] = useState('price_asc');
  // Start with ISR data — no skeleton, no wait on first render
  const [rooms, setRooms] = useState<any[]>(() => sortRooms(initialRooms, 'price_asc'));
  const [loading, setLoading] = useState(false); // false = immediate render
  ...
  const applyFilters = async (sort = sortBy) => {
    setLoading(true);
    try {
  ...
  const handleSortChange = (newSort: string) => {
    setSortBy(newSort);
    setRooms((prev) => sortRooms(prev, newSort));
  };
    ...
    <div className="min-h-screen">
      ...
      <section className="container mx-auto px-4 py-8">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {loading ? 'Recherche…' : `${rooms.length} chambre(s) disponible(s)`}
        </p>

        {/* Loading overlay — only shown during filter re-fetch */}
        {loading && (
              ...
              >
                <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 group border-0 shadow-md">
                  ...
                  <CardContent className="p-5">
                    ...
                    <p className="text-sm text-muted-foreground mb-3 line-clamp-2">{room.shortDescription}</p>
