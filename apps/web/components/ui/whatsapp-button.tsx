...
export function WhatsAppButton() {
  ...
  const quickMessages = [
    ...
    'Avez-vous un service de navette aéroport ?',
  ];
  ...
  return (
    <>
          ...
          >
            ...
            <div className="bg-[#075E54] p-4 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm">Hotel SETIFANA</p>
                    <p className="text-xs text-white/70">En ligne • Répond en ~5min</p>
                  </div>
                </div>
            ...
            <div className="p-4 bg-[#ECE5DD] min-h-[120px]">
              <div className="bg-white rounded-lg px-3 py-2 shadow-sm max-w-[85%]">
                ...
                <p className="text-[10px] text-gray-400 text-right mt-1">Maintenant</p>
              </div>
            </div>

            {/* Quick replies */}
            <div className="p-3 border-t space-y-2">
              <p className="text-xs text-muted-foreground font-medium">Réponses rapides :</p>
              {quickMessages.map((msg, i) => (
                <button
                  key={i}
                  onClick={() => sendMessage(msg)}
                  className="block w-full text-left text-sm px-3 py-2 rounded-lg border hover:bg-green-50 hover:border-green-200 transition-colors"
                >
                  {msg}
                </button>
              ))}
      ...
      <motion.button
        ...
        aria-label={isOpen ? 'Fermer WhatsApp' : 'Ouvrir WhatsApp'}
